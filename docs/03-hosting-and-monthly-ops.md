...
- [ ] `site/CNAME` is documentation-only under the GitHub Actions Pages source (see section 2.4) - kept in the repo; the domain itself is set in the repo Pages settings, at the default `staedean-b-v.github.io` URL; only relevant once/if the `pulse.staedean.com` switch happens, and even then the repo's Settings > Pages value is what actually governs the domain, not this file.
